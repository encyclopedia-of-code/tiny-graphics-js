...
{ material( options )                      // Phong Materials expect you to pass in options like the following:
  ...
  shared_glsl_code()            // ********* SHARED CODE, INCLUDED IN BOTH SHADERS *********
    { return ` precision mediump float;
        uniform float ambient, diffusivity, specularity, smoothness;
        uniform vec4 lightColor, shapeColor;
        uniform vec3 H, E, L squared_scale, camera_center;
        varying vec3 vN, vE, vL;                    // Specifier "varying" means a variable's final value will be passed from the vertex shader 
                                           // on to the next phase (fragment shader), then interpolated per-fragment, weighted by the 
                                           // pixel fragment's proximity to each of the 3 vertices (barycentric interpolation). ` ;
    }
  vertex_glsl_code()           // ********* VERTEX SHADER *********
  ...
  update_GPU( context, gpu_addresses, g_state, model_transform, material )
    { const gpu = gpu_addresses, gl = context;
      ...
      const light_position = g_state.lights[0].position;  // Light position "P" uses homogeneous coords.
      gl.uniform3fv( gpu.light_position_or_vector, light_position_or_vector );

      // Approximate Blinn's value of "H" using an estimate of L and E
      
      const E = camera_center.minus( center ).to3().normalized();
      let L = P[3] ? P.minus( center ) : P;  // Use w = 0 for a directional light source -- a vector instead of a point.
          L = L.to3().normalized();
