...
export class Inertia_Demo extends Physics_Demo {
  ...
  init() {
      ...
      this.num_falling_bodies = 1000;
      this.state.bodies = [];
      this.simulation = new Simulation( { target: this, state: this.state } );
      this.animated_children.push( this.simulation );
  ...
  update_state( dt ) {
      ...
      this.state.bodies = this.state.bodies.filter( b => b.center.norm() < 50 && Math.abs( b.linear_velocity.data[1] ) > .05 );
  }
